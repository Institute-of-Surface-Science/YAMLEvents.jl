...
An error raised when valid YAML tokens cannot form a valid document structure.

`context` and `context_mark` identify the construct being parsed. `problem` and
`problem_mark` identify the offending syntax. Fields that are not relevant to a
particular error are `nothing`.
"""
struct ParserError <: Exception
    ...
    function ParserError(context = nothing, context_mark = nothing, problem = nothing,
                         problem_mark = nothing, note = nothing)
...
mutable struct EventStream
    ...
    function EventStream(input::TokenStream)
        new(input, nothing, parse_stream_start, Function[], Mark[], nothing,
    ...
    while peek(stream.input) isa DirectiveToken
        ...
        if token.name == "YAML"
            if stream.yaml_version !== nothing
                throw(ParserError(nothing, nothing, "found duplicate YAML directive",
                                  firstmark(token)))
            end
        ...
        elseif token.name == "TAG"
            ...
            if haskey(stream.tag_handles, handle)
                throw(ParserError(nothing, nothing, "duplicate tag handle $(handle)",
                                  firstmark(token)))
            end
...
function parse_stream_start(stream::EventStream)
    ...
    event = StreamStartEvent(firstmark(token), lastmark(token), token.encoding)
...
function parse_document_start(stream::EventStream)
    ...
    if !(token isa StreamEndToken)
        ...
        token = forward!(stream.input)
        event = DocumentStartEvent(start_mark, lastmark(token), true, version, tags)
        push!(stream.states, parse_document_end)
...
function parse_document_end(stream::EventStream)
    ...
    if token isa DocumentEndToken
        ...
        explicit = true
        stream.end_of_stream = StreamEndEvent(firstmark(token), lastmark(token))
    end
...
function __parse_node(token::ScalarToken, stream::EventStream, block, start_mark, end_mark,
                      anchor, tag, implicit)
    forward!(stream.input)
    end_mark = lastmark(token)
...
function __parse_node(token::FlowSequenceStartToken, stream::EventStream, block, start_mark,
                      end_mark, anchor, tag, implicit)
    end_mark = lastmark(token)
    stream.state = parse_flow_sequence_first_entry
    SequenceStartEvent(start_mark, end_mark, anchor, tag, implicit, true)
end

function __parse_node(token::FlowMappingStartToken, stream::EventStream, block, start_mark,
                      end_mark, anchor, tag, implicit)
    end_mark = lastmark(token)
...
function __parse_node(token::BlockSequenceStartToken, stream::EventStream, block,
...
function __parse_node(token::BlockMappingStartToken, stream::EventStream, block, start_mark,
                      end_mark, anchor, tag, implicit)
    block || return nothing
    end_mark = firstmark(token)
...
function __parse_node(token, stream::EventStream, block, start_mark, end_mark, anchor, tag,
                      implicit)
    if anchor !== nothing || tag !== nothing
        stream.state = pop!(stream.states)
        return ScalarEvent(start_mark, end_mark, anchor, tag, (implicit, false), "",
                           nothing)
    else
...
function _parse_node(token, stream::EventStream, block, indentless_sequence)
    ...
    if indentless_sequence && token isa BlockEntryToken
        ...
        event = __parse_node(token, stream, block, start_mark, end_mark, anchor, tag,
                             implicit)
    end
...
function _parse_flow_sequence_entry(token::FlowSequenceEndToken, stream::EventStream,
                                    first_entry = false)
    forward!(stream.input)
    pop!(stream.marks)
    stream.state = pop!(stream.states)
...
function _parse_flow_sequence_entry(token::Any, stream::EventStream, first_entry = false)
    if !first_entry
        if token isa FlowEntryToken
            ...
            throw(ParserError("while parsing a flow sequence", stream.marks[end],
    ...
    if isa(token, KeyToken)
        ...
        MappingStartEvent(firstmark(token), lastmark(token), nothing, nothing, true, true)
    ...
    if !(token isa FlowMappingEndToken)
        if !first_entry
            if token isa FlowEntryToken
                ...
                throw(ParserError("while parsing a flow mapping", stream.marks[end],
                                  "expected ',' or '}', but got $(typeof(token))",
                                  firstmark(token)))
            end
        end
