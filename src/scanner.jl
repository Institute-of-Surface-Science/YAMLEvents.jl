...
function is_b_char(::YAMLV1_1, c::Char)
    c == '\n' || c == '\r' || c == '\u85' || c == '\u2028' || c == '\u2029'
end
...
function is_whitespace(::YAMLV1_1, c::Char)
    c == '\0' || c == ' ' || c == '\t' || is_b_char(YAMLV1_1(), c)
end
...
An error raised when the input contains text that cannot be tokenized as YAML.

`context` and `context_mark` identify the scanner operation when available;
`problem` and `problem_mark` describe and locate the invalid text.
"""
...
mutable struct TokenStream
    ...
    function TokenStream(stream::IO)
        ...
        tokstream = new(BufferedInput(decoded_stream), encoding, false, Queue{Token}(), 0,
                        0, 1, 0, 0, -1, Int[], true, Dict())
...
function check_plain(stream::TokenStream)
    !in(peek(stream.input), "\0 \t\r\n\u0085\u2028\u2029-?:,[]{}#&*!|>\'\"%@`\uFEFF") ||
        (!is_whitespace(YAMLV1_1(), peek(stream.input, 1)) && (peek(stream.input) == '-' ||
...
function fetch_block_entry(stream::TokenStream)
    ...
    if stream.flow_level == 0
        ...
        if !stream.allow_simple_key
            throw(ScannerError(nothing, nothing, "sequence entries not allowed here",
                               get_mark(stream)))
        end
...
function fetch_key(stream::TokenStream)
    if stream.flow_level == 0
        ...
        if !stream.allow_simple_key
            throw(ScannerError(nothing, nothing, "mapping keys are not allowed here",
                               get_mark(stream)))
        end
...
function fetch_value(stream::TokenStream)
    ...
    if haskey(stream.possible_simple_keys, stream.flow_level)
        ...
        if stream.flow_level == 0 && add_indent(stream, key.mark.column)
            enqueue!(stream.token_queue, BlockMappingStartToken(Span(key.mark, key.mark)),
                     key.token_number - stream.tokens_taken)
        end
        ...
        if stream.flow_level == 0
            ...
            if !stream.allow_simple_key
                throw(ScannerError(nothing, nothing, "mapping values are not allowed here",
                                   get_mark(stream)))
            end
        end
...
function scan_yaml_directive_number(stream::TokenStream, start_mark::Mark)::Int
    ...
    isdigit(c) || throw(ScannerError("while scanning a directive", start_mark,
...
function scan_block_scalar_indicators(stream::TokenStream, start_mark::Mark)
    ...
    elseif isdigit(c)
        increment = parse(Int, c)
        increment == 0 && throw(ScannerError("while scanning a block scalar", start_mark,
                           "expected indentation indicator in the range 1-9, but found 0",
                           get_mark(stream)))
        forwardchars!(stream)
        c = peek(stream.input)
    ...
    !(c == '\0' ||
      ...
      c == '\u85' ||
      c == '\u2028' ||
      c == '\u2029') && throw(ScannerError("while scanning a block scalar", start_mark,
...
const ESCAPE_REPLACEMENTS = Dict{Char, Char}('0' => '\0', 'a' => '\u0007', 'b' => '\u0008',
                                             't' => '\u0009', '\t' => '\u0009',
                                             'n' => '\u000a', 'v' => '\u000b',
                                             'f' => '\u000c', 'r' => '\u000d',
                                             'e' => '\u001b', ' ' => '\u0020', '"' => '"',
    ...
    while true
        ...
        elseif double && c == '\\'
            ...
            elseif is_b_char(YAMLV1_1(), c)
                ...
                throw(ScannerError("while scanning a double-quoted scalar", start_mark,
    ...
    while true
        ...
        if pref == "---" ||
           pref == "..." && in(peek(stream.input, 3), "\0 \t\r\n\u0085\u2028\u2029")
            throw(ScannerError("while scanning a quoted scalar", start_mark,
                               "found unexpected document seperator", get_mark(stream)))
        end
...
function scan_tag_uri(stream::TokenStream, name::String, start_mark::Mark)
    ...
    if isempty(chunks)
        throw(ScannerError("while parsing a $(name)", start_mark,
                           "expected URI, but found '$(c)'", get_mark(stream)))
    end
...
function scan_uri_escapes(stream::TokenStream, name::String, start_mark::Mark)::String
    ...
    while peek(stream.input) == '%'
        ...
        for k in 0:1
            c = peek(stream.input, k)
            is_ns_hex_digit(c) || throw(ScannerError("while scanning a $name", start_mark,
                               "expected URI escape sequence of 2 hexadecimal digits, but found '$c'",
                               get_mark(stream)))
        end
